...
import org.dynamisengine.gpu.api.upload.GpuRayTracingGeometryUploader;
...
/**
 * Vulkan upload/create path for RT geometry metadata resources.
 */
public final class VulkanRayTracingGeometryUploader implements GpuRayTracingGeometryUploader {
  @FunctionalInterface
  ...
  public VulkanRayTracingGeometryUploader(
  ...
  VulkanRayTracingGeometryUploader(BufferFactory bufferFactory) {
    this.bufferFactory = Objects.requireNonNull(bufferFactory, "bufferFactory");
  }

  @Override
  public GpuRayTracingGeometryResource upload(GpuRayTracingGeometryPayload payload) throws GpuException {
    Objects.requireNonNull(payload, "payload");
    if (payload.regionCount() == 0 || payload.regionsByteSize() == 0) {
      throw new IllegalArgumentException("RT geometry upload requires non-empty payload");
    }
