package org.dynamisgpu.vulkan.sync;
...
public final class VulkanFrameSyncResources {
    ...
    private static long createCommandPool(VkDevice device, MemoryStack stack, int graphicsQueueFamilyIndex) throws GpuException {
        ...
        if (poolResult != VK_SUCCESS || pPool.get(0) == VK_NULL_HANDLE) {
            throw new GpuException(GpuErrorCode.BACKEND_INIT_FAILED, "vkCreateCommandPool failed: " + poolResult, false);
        }
        return pPool.get(0);
    }
    ...
    private static VkCommandBuffer[] allocateCommandBuffers(
        ...
        if (allocResult != VK_SUCCESS) {
            throw new GpuException(GpuErrorCode.BACKEND_INIT_FAILED, "vkAllocateCommandBuffers failed: " + allocResult, false);
        }
    ...
    private static SyncPrimitives createSyncPrimitives(VkDevice device, MemoryStack stack, int framesInFlight) throws GpuException {
        ...
        for (int i = 0; i < framesInFlight; i++) {
            ...
            if (semaphoreResult != VK_SUCCESS || pSemaphore.get(0) == VK_NULL_HANDLE) {
                throw new GpuException(GpuErrorCode.BACKEND_INIT_FAILED, "vkCreateSemaphore(imageAvailable) failed: " + semaphoreResult, false);
            }
            ...
            if (semaphoreResult != VK_SUCCESS || pSemaphore.get(0) == VK_NULL_HANDLE) {
                throw new GpuException(GpuErrorCode.BACKEND_INIT_FAILED, "vkCreateSemaphore(renderFinished) failed: " + semaphoreResult, false);
            }
            ...
            if (fenceResult != VK_SUCCESS || pFence.get(0) == VK_NULL_HANDLE) {
                throw new GpuException(GpuErrorCode.BACKEND_INIT_FAILED, "vkCreateFence failed: " + fenceResult, false);
            }
