...
public final class MeshForgeIngestionReportFormatter {
  ...
  public static String toLine(MeshForgeIngestionRunReport report) {
    ...
    return String.format(
        Locale.ROOT,
        "fixture=%s status=%s source=%s mode=%s completion=%s vertexCount=%s indexCount=%s stride=%s indexType=%s submeshes=%s uploadedBytes=%s uploadGbps=%s cacheHitLoadMs=%.3f bridgeMs=%.3f uploadMs=%s totalMs=%.3f detail=%s",
        report.fixtureName(),
        report.status(),
        report.source(),
        report.uploadMode() == null ? "NA" : report.uploadMode(),
        report.completionMode() == null ? "NA" : report.completionMode(),
        ...
        report.uploadedBytes() > 0L ? Long.toString(report.uploadedBytes()) : "NA",
        report.uploadBandwidthGbps() == null ? "NA" : String.format(Locale.ROOT, "%.3f", report.uploadBandwidthGbps()),
